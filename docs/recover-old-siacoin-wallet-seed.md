...
That's not a "forgotten password" situation or a standard restore — it's a legacy Sia-UI seed from before the Sia v2 hardfork, and it needs the dedicated migration assistant rather than the restore flow. See the migration guide linked below for the full walkthrough.
